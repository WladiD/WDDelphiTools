...
type
  ...
  private
    ...
    ///   The embedded-EXE path scans from 256 (past the DOS/PE stub); the
    ///   standalone .tds path scans from 0 (the signature sits at offset 0).
    /// </summary>
    procedure LoadFromBytesScanning(const ABytes: TBytes; AScanStart: Integer);
  ...
  public
    ...
    /// <summary>
    ///   Loads TD32 debug info from a standalone .tds sidecar (emitted by
    ///   dcc32's -VT option, which keeps the EXE lean by detaching the
    ///   CodeView stream into a separate file). A .tds is the raw TD32
    ///   container, so the FB09 / FB0A signature sits at/near offset 0 —
    ///   the scan therefore starts at 0 rather than past a PE stub.
    /// </summary>
    procedure LoadFromTdsFile(const ATdsPath: String);
    procedure LoadFromTdsBytes(const ABytes: TBytes);
    /// <summary>
...
constructor TTd32Reader.Create;
begin
  inherited Create;
...
procedure TTd32Reader.LoadFromBytes(const ABytes: TBytes);
begin
  // Embedded TD32 inside a Delphi PE: skip the DOS stub and PE optional
  // header where the magic never legitimately appears, so the first hit is
  // the header rather than the EOF trailer.
  LoadFromBytesScanning(ABytes, 256);
end;

procedure TTd32Reader.LoadFromTdsFile(const ATdsPath: String);
var
  Bytes : TBytes;
  Stream: TFileStream;
begin
  Stream := TFileStream.Create(ATdsPath, fmOpenRead or fmShareDenyWrite);
  try
    SetLength(Bytes, Stream.Size);
    if Stream.Size > 0 then
      Stream.ReadBuffer(Bytes[0], Stream.Size);
  ...
  LoadFromTdsBytes(Bytes);
end;

procedure TTd32Reader.LoadFromTdsBytes(const ABytes: TBytes);
begin
  // A standalone .tds is the raw TD32 container; the FB09 / FB0A header is
  // at offset 0, so scan from the very start (the embedded-EXE skip-to-256
  // would step past it).
  LoadFromBytesScanning(ABytes, 0);
end;

procedure TTd32Reader.LoadFromBytesScanning(const ABytes: TBytes;
  AScanStart: Integer);
const
...
begin
  ...
  // Locate the FB09 / FB0A header on a 4-byte boundary starting at
  // AScanStart. The same signature also appears in the 8-byte trailer at
  // EOF, so taking the earliest hit guarantees the header rather than the
  // trailer.
  TD32Start := -1;
  Idx := AScanStart;
  if Idx < 0 then
...
procedure TTd32Reader.ParseStream(AStreamPtr: Pointer; AStreamSize: Integer);
var
...
function TTd32Reader.ParseNamesSubsection(APtr: Pointer; ASize: Integer): TArray<String>;
var
  Count: UInt32;
  P    : PByte;
  PEnd : PByte;
...
procedure TTd32Reader.WalkAlignSymbols(APtr: Pointer; ASize: Integer;
...
procedure TTd32Reader.WalkGlobalTypes(APtr: Pointer; ASize: Integer;
  const ANames: TArray<String>);
...
function TTd32Reader.FindStructByTypeIdx(ATypeIdx: UInt32): Integer;
var
  I: Integer;
begin
...
function TTd32Reader.FindClassByName(const AName: String): Integer;
var
  I: Integer;
begin
  for I := 0 to FClasses.Count - 1 do
...
function TTd32Reader.FindClassMember(const AClassName, AFieldName: String;
  out AMember: TTd32ClassMember): Boolean;
var
  ClsIdx, I: Integer;
  Info     : TTd32ClassInfo;
...
function TTd32Reader.FindProcContaining(ASegmentOffset: UInt32): Integer;
var
  I: Integer;
begin
...
function TTd32Reader.FindProcByName(const AName: String): Integer;
var
  I       : Integer;
  Stored  : String;
  StripAt : String;
