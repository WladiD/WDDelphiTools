  ...
  begin
    FActions := [
      ...
      TActionInfo.Create('ExeAnalyze', '<ExeFile> [--Format=Text|Json] [--UnitsOnly] [--Index=<file>] [--SortBy=VA|Name|Size]', [
        ...
        'subsystem, compile timestamp, sections) and lists every linked',
        'unit. Detection methods, in descending order of confidence:',
        '  1. Sibling .map file (same base name, .map extension) - the',
        '     most reliable source; gives every unit plus per-unit code',
        '     size from the segment table.',
        '  2. Embedded TD32 debug info (FB09/FB0A signature) - present',
        '     in any binary built with DCC_DebugInformation=true. Gives',
        '     the full module list but no per-unit code size.',
        '  3. Heuristic identifier scan of the EXE bytes - last resort',
        '     for stripped binaries. Captures dotted unit names only',
        '     (System.SysUtils etc.) to keep the false-positive rate',
        '     low; old non-namespaced units like JclMath are missed.',
        'The detection method that actually fired is reported in the',
        '[Detection] block of the output (map / td32 / rtti-scan).',
        'Options:',
        '  --Format=Text (default) or --Format=Json for machine output.',
        '  --UnitsOnly suppresses every section except the bare unit-name',
        '    list, one name per line. Designed for shell pipelines:',
        '      DPT ExeAnalyze MyApp.exe --UnitsOnly | grep Spring',
        '  --Index=<file> cross-references unit names against a',
        '    previously built DcuIndex JSON. Each unit then carries the',
        '    on-disk DCU path that contributed it - the build BoM. When',
        '    the index has multiple platform variants of a unit, the one',
        '    matching the EXE''s platform wins.',
        '  --SortBy=VA|Name|Size  controls the order of the linked-units',
        '    list. Default is VA (= linker layout order, lowest-address',
        '    first). "Name" is case-insensitive alphabetical. "Size" is',
        '    descending (largest first), useful for spotting code-bloat',
        '    contributors. Equal sizes break the tie alphabetically.',
        'Example:',
        '  DPT ExeAnalyze MyApp.exe',
        '  DPT ExeAnalyze MyApp.exe --Index=lib.json --Format=Json'
      ]),
