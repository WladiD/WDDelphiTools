...
type
  ...
  /// Re-exported from DPT.Debugger.Types (hoisted there to break the
  /// TDebugger <-> TVariableEvaluator unit cycle). Aliased here so the
  /// many existing `DPT.Debugger.TLocalVar` / `TEvaluateFormatter` users
  /// keep compiling without each adding the new unit to their uses.
  TLocalVar = DPT.Debugger.Types.TLocalVar;
  TEvaluateFormatter = DPT.Debugger.Types.TEvaluateFormatter;
  ...
  TDebugger = class(TObject, IEvalProcessAccess)
  private
    ...
    // --- IEvalProcessAccess (the variable-evaluator seam) -------------
    // No-op reference counting: TDebugger is a plain object whose
    // lifetime the owner controls; holding it as IEvalProcessAccess must
    // not free it. QueryInterface still resolves the seam.
...
begin
  ...
  // The evaluator drives off Self via IEvalProcessAccess (no-op
  // refcounted, so this interface reference does not own the lifetime).
  FEvaluator := TVariableEvaluator.Create(Self);
end;
...
// --- IEvalProcessAccess implementation --------------------------------
// No-op reference counting: TDebugger's lifetime is owned by its creator,
// not by interface references the evaluator holds.
function TDebugger.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then Result := S_OK
  else Result := E_NOINTERFACE;
end;

function TDebugger._AddRef: Integer;
begin
  Result := -1;
end;

function TDebugger._Release: Integer;
begin
  Result := -1;
end;

function TDebugger.GetTargetIs32Bit: Boolean;
begin
  Result := FTargetIs32Bit;
end;

function TDebugger.GetTargetPointerSize: Integer;
begin
  Result := FTargetPointerSize;
end;

function TDebugger.GetLastThreadHit: THandle;
begin
  Result := FLastThreadHit;
end;

function TDebugger.GetBaseAddress: UIntPtr;
begin
  Result := FBaseAddress;
end;

function TDebugger.GetCallResultSlot: UIntPtr;
begin
  Result := FCallResultSlot;
end;

function TDebugger.GetLastEnumTypeId: UInt32;
begin
  Result := FLastEnumTypeId;
end;

procedure TDebugger.SetLastEnumTypeId(AValue: UInt32);
begin
  FLastEnumTypeId := AValue;
end;
...
function TDebugger.GetPrimitiveTypeFormatters: IKeyValue<UInt16, String>;
begin
  Result := FPrimitiveTypeFormatters;
end;

// The variable-evaluation engine now lives in TVariableEvaluator
// (DPT.Debugger.Evaluate). EvaluateVariable delegates to it; the
// evaluator calls back through Self's IEvalProcessAccess seam.
function TDebugger.EvaluateVariable(const AName: String; var AType: String;
  out AValue, AHint: String; AOnPhase: TProc<String>): Boolean;
begin
  Result := FEvaluator.Evaluate(AName, AType, AValue, AHint, AOnPhase);
end;
