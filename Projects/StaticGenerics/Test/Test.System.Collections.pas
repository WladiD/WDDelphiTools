...
begin
  ...
  var FirstItem: String := 'Hello World!';
  List.Add(FirstItem);
  ...
  var SecondItem: String := 'Some random content';
  List.Add(SecondItem);
  Assert.AreEqual(2, List.Count);
  Assert.AreEqual(FirstItem, List.First);
  Assert.AreEqual(SecondItem, List.Last);

  var ThirdItem: String := 'Lorem ipsum et delomet';
  List.Add(ThirdItem);
  ...
  for var CurItem: String in List do
  begin
  ...
  for var Loop: Integer := 1 to TestLoops do
  begin
    var LoopValue: String := 'MyContent-' + IntToStr(Loop);
    var AddedIndex: Integer := List.Add(LoopValue);
  ...
  var EntriesArray: TArray<String> := List.ToArray;
  Assert.AreEqual(Length(EntriesArray), List.Count);

  for var CurItem: String in EntriesArray do
    Assert.IsTrue(List.Contains(CurItem));
end;

procedure TestList_String.SortTest;
begin
  var List: IList_String := TCollections.CreateList_String;
  List.Add('Cello');
  List.Add('Alpha');
  List.Add('Bello');
  List.Sort;
...
begin
  var UniqueList: IList_String := TCollections.CreateListUnique_String;
...
begin
  ...
  for var LoopItem: TObject in OList do
  begin
    if LoopItem is TObject then
      Inc(IsCounter);
  end;
...
begin
  ...
  var IsCounter: Integer := 0;
  for var LoopItem: TComponent in CompList do
  begin
    if LoopItem is TComponent then
      Inc(IsCounter);
  end;
  Assert.AreEqual(3, IsCounter);
end;
...
var
  ...
  begin
    ...
    for var Loop: Integer := 0 to Length(AExpectedComponents) - 1 do
      Assert.AreEqual(AExpectedComponents[Loop], Target[Loop]);
  end;
...
begin
  ...
  for var Loop: Integer := 1 to TestLoops do
    Dict.Add('MyKey-' + IntToStr(Loop), 'MyContent-' + IntToStr(Loop));
...
begin
  ...
  InitObjsAndAddToDict;
  FreeCallCount := 0;
  for var Key: String in Dict.Keys.ToArray do
    Dict.Remove(Key);
  Assert.AreEqual(3, FreeCallCount);
end;
