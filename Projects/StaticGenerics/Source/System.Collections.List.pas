...
begin
  ...
  if AIndex1<>AIndex2
    then inherited Exchange(AIndex1,AIndex2);
end;
...
begin
  ...
  for var Loop: Integer:=0 to Count-1
    do ListObj.Add(Self.Items[Loop]);
  Result:=ListObj;
end;
...
begin
  if
    (IsEqualGUID(IID,FListIntfGuid) and GetInterface(IList_TObject,Obj)) or
...
begin
  Result:=0;
  for var Loop:=Count-1 downto 0 do
  begin
...
begin
  ...
  begin
    ...
    if EndIndex>=Count
      then EndIndex:=Count-1;
  end else EndIndex:=Count-1;
  SetLength(Result,EndIndex-AOffset+1);
  for var Loop:=AOffset to EndIndex
...
begin
  ...
  if ResultList.Any
    then Result:=ResultList as IEnumerable_TObject;
end;
...
begin
  ...
  if Result=1 // Eins, da wir selbst eine Referenz darauf halten
    then Destroy;
end;
